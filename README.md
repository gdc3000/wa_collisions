...
`vehicle_collisions/vehicle_collisions.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `vehicle_collisions.py` in directory called
`vehicle_collisions`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import vehicle_collisions as sb` in an
interactive Python session, the classes and functions defined inside of the
`vehicle_collisions.py` file are available in the `sb` namespace. For this to work, we
...
everything in that file into the namespace of the project:
    ...
    from .vehicle_collisions import *
...
In the case of the vehicle_collisions module, the main classes defined at the bottom of
...
Either way, you can create a `vehicle_collisions/data` folder in which you can
...
the data at:
    ...
    import vehicle_collisions as sb
...
(typically, something like our `vehicle_collisions/tests/test_vehicle_collisions.py`). Within each
...
have one such test in `vehicle_collisions/tests/test_vehicle_collisions.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/vehicle_collisions`),
and type:

    py.test vehicle_collisions

This will exercise all of the tests in your code directory. If a test fails, you
will see a message such as:


    vehicle_collisions/tests/test_vehicle_collisions.py .F...
    ...
    vehicle_collisions/tests/test_vehicle_collisions.py:49: AssertionError
...
To document `vehicle_collisions` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/vehicle_collisions/blob/master/doc/_static/RTD-advanced-conf.png)

 http://vehicle_collisions.readthedocs.org/en/latest/
...
`vehicle_collisions` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `vehicle_collisions`, we use the
...
[here](https://github.com/uwescience/vehicle_collisions/blob/master/scripts/Figure1.ipynb).
